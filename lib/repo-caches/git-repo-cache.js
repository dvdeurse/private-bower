...
module.exports = function GitRepoCache(options) {
    ...
    function _init() {
        return _createDirectory(options.repoCacheRoot)
            ...
            .fail(function(err) {
                logger.error('Failed to initialize public repository cache: ' + err);
	...
	gitCommand += " && git branch -a | grep -v HEAD | perl -ne 'chomp($_); s|^\*?\s*||; if (m|(.+)/(.+)| && not $d{$2}) {print qq(git branch --track $2 $1/$2\n)} else {$d{$_}=1}' | bash -xfs"
        ...
        exec(gitCommand, function(error, stdout, stderr) {
            ...
            //    return;
            //}
	    logger.log('Cloned {0} full git repository to private'.format(repoName));
	    deferred.resolve();
        });
